...
def progress_bar(current, total, prefix="", width=20):
...
def _input_one(name):
    ...
    while True:
        try:
            ...
            progress_bar(1, 3, "读取参数 ")
            tr = input("    厚度 (mm)（圆柱棒直接回车）: ").strip()
            cyl = tr == ''
            t   = w if cyl else float(tr)
            progress_bar(2, 3, "读取参数 ")
            g   = float(input("    标距 (mm): "))
            progress_bar(3, 3, "读取参数 ")
            break
        except ValueError:
            print("    ✗ 请输入数字")
...
def _load_template(path, specimens):
    ...
    for row in ws.iter_rows(min_row=3, values_only=True):
        ...
        loaded += 1
        progress_bar(loaded, len(sp_names), "读取参数 ")
...
def write_result_xlsx(results, prefix):
    ...
    def _write(path):
        ...
        # 各试样 Sheet
        total_save = len(results)
        for idx, (name, data) in enumerate(results.items(), 1):
            progress_bar(idx, total_save, "保存进度 ")
    ...
    log(f"\n  结果：{out}（{len(results)+2} 个工作表）")

def main():
    log(f"\n===== 拉伸试验数据处理 {VERSION} =====")
    start = input("\n  CSV 文件路径：").strip().strip('"')
    if not os.path.exists(start):
        log("  ✗ 文件不存在"); sys.exit(1)
