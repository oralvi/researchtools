...
def get_all_specimen_params(specimens: list) -> dict:
    ...
    for sp_name, _ in specimens:
        ...
        if is_cylinder:
            ...
            print(f"    → 圆柱棒，S = π/4 × {width}² = {area:.4f} mm²")
        elif abs(width - thickness) < 1e-9:
            ans = input(f"    宽度=厚度={width}mm，是否为圆柱棒？[y/N]: ").strip().lower()
            if ans == 'y':
                area = width * thickness * math.pi / 4
                print(f"    → 圆形截面 S = {area:.4f} mm²")
            else:
                area = width * thickness
                print(f"    → 矩形截面 S = {area:.4f} mm²")
        else:
            area = width * thickness
            print(f"    → 矩形截面 S = {area:.4f} mm²")
